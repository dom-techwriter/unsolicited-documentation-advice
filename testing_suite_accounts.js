...
pm.test("Return type and in the correct format", function () {
    pm.expect(response.type, "type isn't returned").to.be.oneOf([null, "string"]);
});
...
if (loanData !== null) {
    ...
    pm.test("just testing", function () {
        pm.expect(response.funds_data).to.not.be.null;
    });
    
} else if (response.credit !== null) {
    pm.test("just testing", function () {
        pm.expect(response.credit).to.not.be.null;
    });
}
